...
- 5-10 vineyard AOIs are versioned as manifests or fixtures, with CRS and crop metadata.
- Each AOI has enough Sentinel-2 support after masking or is explicitly rejected.
...
- No supervised CatBoost trigger unless credible labels or loss history exist.
- No claim of reliable intra-row segmentation from Sentinel-2 10 m pixels alone.
- No production billing, self-serve dashboard, or multi-tenant admin UI.
- No Sentinel Hub migration until the Planetary Computer numerical path is stable.
...
- AOI batch manifest schema for vineyard test parcels.
- 5-10 candidate AOIs across Bordeaux and/or Languedoc.
- Crop-mask/RPG input convention and CRS handling documented.
- Geometry and raster support checks for min area, min pixels, CRS, and invalid shapes.
- Rejection reasons surfaced before expensive Sentinel-2 loads where possible.

Exit criteria:

- `orbitrisk` can validate a batch manifest locally.
- Tiny or invalid AOIs fail with actionable reasons.
- At least one AOI has an external crop mask ready for the benchmark.
...
- Same-day-of-year seasonal baseline metadata for each accepted period.
